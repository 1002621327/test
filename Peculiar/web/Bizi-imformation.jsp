...
<body>
    ...
    <h4 id="denlu">
    		<form action="Find.jsp" method="post" onchange="submit()">
    			...
    			<a onClick="fun('oneupload.jsp')">上传</a>       </h4> 
    	</form>
    	</div>
	...
	while(iter.hasNext()){
		...
		if(id.equals(ID)){
			%>
			<img src="./bizi/<%=id%>" width=60% >
			<table border="0.1px" cellspacing="0" align="right" width="40%" >
			
			<tr> <td width="12%">ID:</td> <td><%= ID %></td> </tr>
			<tr> <td>名称：</td> <td><%= bizi.getTitle() %></td> </tr>
			<tr> <td>类型：</td> <td><%= bizi.getStyle() %></td> </tr>
			<tr> <td>时间：</td> <td><%= bizi.getTime() %></td> </tr>
			<tr> <td>内容：</td> <td><%= bizi.getBody() %></td> </tr>
